...
my @dat = `query-pr --expr ' ($query_originator)  &  (Created > "$str2")  &  (Created < "$str3")' --format '"%s::&&::%s::&&::%s::&&::%s::&&::%s::&&::%s::&&::%Q::&&::%s::&&::%s::&&::%s::&&::%s::&&::%s::&&::%s::&&::%s"  Number State Resolution Planned-Release Category Resolution-Reason Created Originator Attributes Last-Known-Working-Release Symptom Problem-Level Found-During Synopsis'`;
...
if ($lgth) {
   ...
   print "$tmp[0],$tmp[1],$tmp[2],$planned,$cate,$resolutionres,$date,$manager,$originator\n";
        ...
        my $query2 = "update pr_tracking_new set state='$tmp[1]', category = '$cate',attribute='$attribute', resolution_reason='$resolutionres',scope ='$scope' where number='$tmp[0]'";
        my $stmnt = $dbh2->do($query2)or die $DBI::errstr;
        if ($stmnt == 0){
            my $query4 = "insert into pr_tracking_new (number,state,resolution,planned_release,category,resolution_reason,created,manager,originator,scope,heirarchy,attribute,last_known,symptom,problem_level,found_during,synopsis) values('$tmp[0]','$tmp[1]','$tmp[2]','$planned','$cate','$resolutionres','$date','$manager','$originator','$scope','$heirarchy','$attribute','$last_known','$symptom','$problem_level','$found_during','$synopsis')";
            my $dqh_insert = $dbh2->prepare($query4);
            my $rv = $dqh_insert->execute();
        }
    }  
}
